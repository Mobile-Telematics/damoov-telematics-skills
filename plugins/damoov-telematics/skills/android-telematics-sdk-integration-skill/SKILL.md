...
- If the host app does not need another `Application` base class, use `TelematicsSDKApp` — a convenience `Application` subclass provided by the SDK that calls `TrackingApi.getInstance().initialize(...)` in `onCreate()` with preset `Settings` (high accuracy, auto-start on, passive detection on). Override `setTelematicsSettings()` to customize. If the app needs a different base class (e.g. `MultiDexApplication`, a DI-framework base class, or `FlutterApplication`), inherit from `Application` directly and call `TrackingApi.getInstance().initialize(this, settings)` yourself in `onCreate()`. Do not extend `TelematicsSDKApp` and also call `initialize(...)` manually — it will double-initialize.
...
- Configure persistent intervals with `setMaxPersistentTrackingInterval(minutes)` only in the valid `5..600` minute range; default is `240` minutes (4 hours). This is the automatic stop threshold: persistent tracking stops at whichever comes first — an explicit `stopTracking()` call OR the interval expiry. Example: `setMaxPersistentTrackingInterval(60)` → tracking auto-stops after 60 minutes if not manually stopped earlier.
...
see `../../shared/references/devicetoken-flow.md`.

- **Production registration** (server-side): `../damoov-backend-registration-skill/SKILL.md`
- **Test DeviceToken** (quick setup): `../damoov-integration-testing-skill/SKILL.md`

## After Tracking Is Set Up

To fetch trips, safety scores, and driving statistics via REST API using the DeviceToken:
see `../damoov-trips-statistics-skill/SKILL.md`.
