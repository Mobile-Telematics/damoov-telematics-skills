...
Use `api-migration.md` for replacement details.
...
- `RPFutureTag.init(tag:source:)` requires `tag: String` and accepts nullable `source: String?`.
- `tag` and `source` are product-defined values. The SDK does not impose a fixed enum or SDK-side value list.
...
- When there is no source, pass `nil`; do not invent an empty string:

```swift
let futureTag = RPFutureTag(tag: "business", source: nil)
```
