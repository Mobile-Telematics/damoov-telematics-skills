...
- `setMaxPersistentTrackingInterval(minutes:)` accepts values in the `5..600` minute range and throws on values outside that range; handle the error before starting persistent tracking. The default is 240 minutes (4 hours). Persistent tracking stops at whichever comes first — an explicit `stopTracking()` call or the interval expiry — then the SDK waits for the next start.
