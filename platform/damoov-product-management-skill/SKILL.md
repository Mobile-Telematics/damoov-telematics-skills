...
> Hierarchy READ methods already documented in `../../backend/damoov-backend-registration-skill/references/hierarchy.md`:
...
For Admin JWT acquisition see `../../backend/damoov-backend-registration-skill/SKILL.md` (Step 1)
or `../../testing/damoov-integration-testing-skill/references/admin-auth.md`.
...
`../../backend/damoov-backend-registration-skill/references/hierarchy.md` — Branch C.
...
1. **Enumerate users per Instance** using `POST /v1/Management/users/GetFilteredPage` with body `{"Filters": [{"Name": "InstanceId", "Value": "<InstanceId>"}]}`.
   Full curl and response: `../../testing/damoov-integration-testing-skill/references/registration-api.md` — "GetFilteredPage" section.
