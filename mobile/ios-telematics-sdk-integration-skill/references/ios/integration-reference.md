...
`RPFutureTag.init(tag:source:)` requires `tag: String` and accepts nullable `source: String?`. Both values are product-defined; the SDK does not define an enum or impose SDK-side restrictions on them. Use `tag` for the business label and `source` for the app module or user action that created it. When there is no source, pass `nil` rather than an invented empty string.
...
let tag = RPFutureTag(tag: "TAG", source: nil)
...
iOS Simulator can exercise integration flow, permissions, and trip recording — but only if a
location feed is active. **By default the simulator emits no location**, so without enabling
simulation the SDK receives no coordinates and records no trip (`ActivityStatus` stays
`"No Data"` and no trip appears in Datahub).

To feed location on the simulator:
- Menu: **Features → Location → Freeway Drive** (built-in highway-drive preset), or
  City Run / City Bicycle Ride for other profiles.
- CLI (auto-start, preferred by the skill): play an interpolated route —
  `xcrun simctl location booted start --speed=25 37.3324,-122.0332 37.3639,-122.0450 37.3939,-122.0800 37.4139,-122.1100`.
- If `simctl` reports no booted device, ask the developer to pick a preset from the
  **Features → Location** menu.

HF Data (accelerometer/gyroscope) cannot be fully tested on the simulator; run final
background and sensor-heavy validation on a real device.
